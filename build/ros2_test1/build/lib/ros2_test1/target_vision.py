...
POSITION_REPORT_RE = re.compile(r"ID1=(-?\d+)\s+ID2=(-?\d+)")
...
class AbsoluteServoBridge:
    ...
    def send_targets(self, id1=None, id2=None, id4=None):
        ...
        commands = []
        if id1 is not None:
            commands.append(f"id1={int(id1)}")
        if id2 is not None:
            commands.append(f"id2={int(id2)}")
        if id4 is not None:
            commands.append(f"zp4:{int(id4)}")
        if not commands:
            return self.status
        ...
        try:
            ...
            for command in commands:
                ...
                response = self._read_text(0.12).strip()
                if response:
                    responses.append(response.replace("\r", " ").replace("\n", " "))
            response_text = "" if not responses else " | " + " | ".join(responses)
            self.status = "sent " + ",".join(commands) + response_text
        except OSError as exc:
            self.status = f"servo bridge write failed: {exc}"
            print(self.status)
        return self.status

    def query_positions(self, timeout_s=0.45):
        if not self.enabled or self.fd is None:
            return None
        try:
            self._read_text(0.02)
        ...
        self.status = f"feedback ID1={self.last_feedback[0]} ID2={self.last_feedback[1]}"
...
class RedSquareGraspController:
    def __init__(
        ...
        self.synchronized = not (enabled and servo_bridge.write_enabled)
    ...
    def _post_center_plan(self, target=None, frame_shape=None):
        offsets = None
        ...
        if offsets is not None:
            ...
            # Treat camera-target as the hypotenuse and camera-gripper as the
            # measured offset side. This converts vision distance into the
            # gripper's required travel along the arm plane.
            gripper_to_target_mm = math.sqrt(
                max(0.0, camera_to_target_mm * camera_to_target_mm - camera_to_gripper_mm * camera_to_gripper_mm)
            )
            move_x_mm = gripper_to_target_mm - self.target_gripper_distance_mm
            move_z_mm = self.camera_gripper_vertical_offset_mm - offsets["vertical_mm"] - self.post_center_down_mm
        ...
        plan = {
            "current_x_mm": current_x_mm,
            "current_z_mm": current_z_mm,
            "forward_mm": move_x_mm,
            "vertical_target_mm": move_z_mm,
        ...
        return plan, (
            ...
            f"move x={move_x_mm:.0f}mm z={move_z_mm:.0f}mm: "
    ...
    def _update_one_shot(self, target, frame_shape):
        ...
        if self.id4 != self.id4_open:
            ...
            return self._send("one-shot open claw", require_feedback=False)
        ...
        if self.id4 != self.id4_closed:
            ...
            return self._send("one-shot close claw", require_feedback=False)
    ...
    def update(self, target, frame_shape):
        ...
        if target is None:
            ...
            if not self.servo_bridge.write_enabled:
                self.arm_preview.set_targets(expand_id1, expand_id2, self.id4)
                return (
                    f"preview searching red target; slow expand "
                    f"ID1={expand_id1} ID2={expand_id2}"
                )
            if can_command and (expand_id1 != self.id1 or expand_id2 != self.id2):
                self.id1, self.id2 = expand_id1, expand_id2
        ...
        can_command = now - self.last_command_time >= self.command_interval_s
        centered = abs(error_x) <= self.center_deadband_px and abs(error_y) <= self.center_deadband_px

        if not centered:
            self.centered_frames = 0
            self.state = "centering"
            delta_id2 = self._limited_delta(error_x * self.id2_pixel_gain)
            # Positive dy means the target is below the center line; increasing
            # ID1 moves the arm in the direction requested for vertical centering.
            delta_id1 = self._limited_delta(error_y * self.id1_pixel_gain_y)
            if not self.servo_bridge.write_enabled:
                preview_id1 = self._clamp(self.id1 + delta_id1, self.id1_limits)
                preview_id2 = self._clamp(self.id2 + delta_id2, self.id2_limits)
                ...
                self.arm_preview.set_targets(preview_id1, preview_id2, self.id4)
                return (
                    f"preview centering{edge_note} dx={error_x:.0f} dy={error_y:.0f} "
                    f"dID1={delta_id1} dID2={delta_id2} "
                    f"planned ID1={preview_id1} ID2={preview_id2}"
                )
            if can_command and (delta_id1 or delta_id2):
                self.id1 = self._clamp(self.id1 + delta_id1, self.id1_limits)
                self.id2 = self._clamp(self.id2 + delta_id2, self.id2_limits)
                return self._send(
                    f"centering{edge_note} dx={error_x:.0f} dy={error_y:.0f} "
                    f"dID1={delta_id1} dID2={delta_id2}"
                )
            self.arm_preview.set_targets(self.id1, self.id2, self.id4)
            return f"centering{edge_note} dx={error_x:.0f} dy={error_y:.0f}"

        self.centered_frames += 1
        if self.centered_frames < self.stable_frames_required:
            self.state = "confirming center"
            self.arm_preview.set_targets(self.id1, self.id2, self.id4)
            return f"center confirm {self.centered_frames}/{self.stable_frames_required}"

        if not self.post_center_move_complete:
            plan, plan_text = self._post_center_plan(target, frame_shape)
            self.arm_preview.publish_plan_marker(plan, plan_text)
            if plan["ik_error_mm"] > self.post_center_ik_error_mm:
                self.state = "post-center unreachable"
                self.arm_preview.set_targets(self.id1, self.id2, self.id4)
                return f"post-center IK failed: {plan_text}"
            if not self.servo_bridge.write_enabled:
                self.state = "preview post-center move"
                self.arm_preview.set_targets(plan["id1"], plan["id2"], self.id4)
                return f"preview {plan_text}"
            if can_command:
                self.id1 = self._clamp(plan["id1"], self.id1_limits)
                self.id2 = self._clamp(plan["id2"], self.id2_limits)
                self.post_center_move_complete = True
                self.state = "post-center move"
                return self._send(plan_text, require_feedback=True)
            self.arm_preview.set_targets(self.id1, self.id2, self.id4)
            return "post-center waiting command interval"

        if not self.servo_bridge.write_enabled:
            if gripper_distance_mm is None:
                self.state = "preview waiting distance"
                return "preview centered; waiting distance estimate"
            distance_error = gripper_distance_mm - self.target_gripper_distance_mm
            current_x_mm, current_z_mm = gripper_position_mm(self.id1, self.id2)
            planned_id1, planned_id2, ik_error_mm = solve_gripper_position(
                current_x_mm + distance_error,
                current_z_mm,
                self.id1,
                self.id2,
                self.id1_limits,
            ...
            self.state = "preview plan"
            self.arm_preview.set_targets(self.id1, self.id2, self.id4)
            return (
                f"preview IK ID1={planned_id1} ID2={planned_id2} "
                f"open={self.id4_open} close={self.id4_closed} error={ik_error_mm:.1f}mm"
            )

        if self.id4 != self.id4_open and can_command:
            self.state = "open claw"
            self.id4 = self.id4_open
            return self._send("open claw before approach")

        if gripper_distance_mm is None:
            self.state = "waiting distance"
            self.arm_preview.set_targets(self.id1, self.id2, self.id4)
            return "centered; waiting distance estimate"

        distance_error = gripper_distance_mm - self.target_gripper_distance_mm
        if abs(distance_error) > self.distance_deadband_mm:
            self.state = "approaching"
            current_x_mm, current_z_mm = gripper_position_mm(self.id1, self.id2)
            approach_step_mm = max(-20.0, min(20.0, distance_error))
            solved_id1, solved_id2, ik_error_mm = solve_gripper_position(
                current_x_mm + approach_step_mm,
                current_z_mm,
                self.id1,
                self.id2,
                self.id1_limits,
            ...
            if ik_error_mm > 8.0:
                self.state = "unreachable"
                self.arm_preview.set_targets(self.id1, self.id2, self.id4)
                return f"IK unreachable error={ik_error_mm:.1f}mm"
            next_id1 = self._clamp(
                self.id1 + self._limited_delta(solved_id1 - self.id1),
                self.id1_limits,
            )
            next_id2 = self._clamp(
                self.id2 + self._limited_delta(solved_id2 - self.id2),
                self.id2_limits,
            )
            if can_command and (next_id1 != self.id1 or next_id2 != self.id2):
                self.id1, self.id2 = next_id1, next_id2
                return self._send(
                    f"IK approach distance={gripper_distance_mm:.0f}mm error={ik_error_mm:.1f}mm"
                )
            self.arm_preview.set_targets(self.id1, self.id2, self.id4)
            return f"approach wait gripper_distance={gripper_distance_mm:.0f}mm"

        if self.id4 != self.id4_closed and can_command:
            self.state = "close claw"
            self.id4 = self.id4_closed
            return self._send("close claw at target")

        self.state = "grasp hold"
        self.arm_preview.set_targets(self.id1, self.id2, self.id4)
        return f"grasp hold gripper_distance={gripper_distance_mm:.0f}mm"
...
class ArmPreviewPublisher:
    def __init__(self, enabled, id1, id2, id4):
        self.enabled = enabled
        self.id1 = id1
        self.id2 = id2
        self.id4 = id4
    ...
    def set_targets(self, id1, id2, id4):
        self.id1 = int(id1)
        self.id2 = int(id2)
        self.id4 = int(id4)
    ...
    def publish(self, detail, target=None):
        if not self.enabled:
            return
        ...
        try:
            ...
            msg.position = arm_joint_positions(self.id1, self.id2, self.id4)
        ...
        for det in detections:
            ...
            if det["kind"] == "ball":
                ...
                if det.get("source") == "qr":
                    ...
                    label = f"{color} square QR {confidence:.0f}%"
    ...
    def _detect_qr(self, frame):
        ...
        if zbar_decode is not None:
            ...
            for barcode in zbar_decode(gray, symbols=symbols):
                ...
                results.append(self._qr_as_square(color, points, decoded, confidence))
            return results
            ...
            try:
                ...
                if ok and points is not None:
                    for idx, qr_points in enumerate(points):
                        ...
                        color = self._color_from_qr_text(decoded)
                        confidence = 100.0 if color else 0.0
                        if color is None:
                            color, confidence = self._match_qr_template(gray, np.asarray(qr_points))
                        if color in ("red", "blue"):
                            results.append(self._qr_as_square(color, qr_points, decoded, confidence))
            except cv2.error:
                pass
        ...
        if not results:
            try:
                ...
                if points is not None:
                    ...
                    confidence = 100.0 if color else 0.0
                    if color is None:
                        color, confidence = self._match_qr_template(gray, np.asarray(points))
                    if color in ("red", "blue"):
                        results.append(self._qr_as_square(color, points, decoded, confidence))
            except cv2.error:
                pass
        ...
        while True:
            ...
            selected_targets = [
                ...
                and (
                    ...
                    and det.get("kind") in {"ball", "square"}
            ...
            if auto_grasp_enabled:
                ...
                arm_preview.publish(grasp_info, preview_target)

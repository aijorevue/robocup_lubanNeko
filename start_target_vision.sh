...
case "$MODE" in
  ...
  rc|rct6)
    ...
    ;;
  *)
    echo "Unknown SERVO_MODE=$MODE. Use rk or rc."
    exit 2
    ;;
esac
...
pkill -u "$(id -un)" -f "[r]ed_square_grasp_all.launch.py|[r]ed_square_grasp_rc.launch.py|[r]ed_square_grasp_rk_direct.launch.py" 2>/dev/null || true

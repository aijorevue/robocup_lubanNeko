...
class RedSquareGraspController:
    ...
    def _send_gripper_id7(self, target, reason):
        ...
        # ID7 remains a fast 250 ms move, but repeat the short bus frame once
        # so a UART/bus timing edge cannot lose the pulse command.
        self.status = self.servo_bridge.send_targets(id4=target, repeat=2)
        self.last_command_time = time.monotonic()
        if self.servo_bridge.write_enabled and not self.servo_bridge.last_command_ok:
    ...
    def update_chassis_station(
        ...
        if self.chassis_station_stage == "disc_close_wait":
            ...
            self.chassis_station_stage = (
                "disc_route_splitter"
                if self.disc_pending_splitter_id4 is not None
                else "disc_detect"
            )

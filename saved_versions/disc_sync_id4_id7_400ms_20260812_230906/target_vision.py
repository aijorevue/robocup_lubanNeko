...
DISC_CATCH_SPLITTER_SETTLE_S = 1.0
...
class RedSquareGraspController:
    def __init__(
        ...
        self.splitter_settle_deadline = 0.0
        self.splitter_target_pending = None
    ...
    def _prepare_disc_catch_splitter_before_claw(self, now):
        ...
        if self.splitter_target_pending != target:
            self.splitter_target_pending = target
            self.splitter_settle_deadline = 0.0

        if self.splitter_id4 != target:
            if now - self.last_aux_command_time < self.aux_command_interval_s:
                return (
                    f"disc catch waiting splitter command interval "
                    f"ID4={self.splitter_id4}->{target}"
                )
            color = self.locked_target.get("color", "ball")
            reason = f"disc catch {color} ball; set splitter before ID7"
            result = self._send_splitter_id4(target, reason)
            if self.servo_bridge.write_enabled and not self.servo_bridge.last_command_ok:
                self.algorithm_stage = "fault"
                self.state = "fault"
                self.status = f"disc catch splitter failed before ID7: {result}"
                return self.status
            self.splitter_settle_deadline = now + DISC_CATCH_SPLITTER_SETTLE_S
            return (
                f"{result}; waiting {DISC_CATCH_SPLITTER_SETTLE_S:.1f}s "
                "for ball to enter channel"
            )

        if self.splitter_settle_deadline <= 0.0:
            self.splitter_settle_deadline = now + DISC_CATCH_SPLITTER_SETTLE_S

        if now < self.splitter_settle_deadline:
            return (
                f"disc catch splitter ID4={self.splitter_id4} settling "
                f"{self.splitter_settle_deadline - now:.1f}s before ID7"
            )
    ...
    def _update_locked_grasp(self, frame_shape, now, can_preview_step, live_target=None):
        ...
        if self.algorithm_stage == "open":
            ...
            splitter_wait = self._prepare_disc_catch_splitter_before_claw(now)
            if splitter_wait is not None:
                self.arm_preview.set_targets(self.id1, self.id2, self.id4, self.id6)
                return splitter_wait
            if not self.servo_bridge.write_enabled:

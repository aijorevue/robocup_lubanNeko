...
DISC_CATCH_COLOR_CONFIRM_FRAMES = 3
DISC_CATCH_COLOR_JUMP_RESET_PX = 90.0
...
class ArmTuneFileBridge:
    ...
    def _execute(self, command_text, controller, chassis_link, frame_shape):
        ...
        if not controller.servo_bridge.ready_for_commands():
...
class RedSquareGraspController:
    def __init__(
        ...
        self.disc_candidate_color = None
        self.disc_candidate_center = None
        self.disc_candidate_frames = 0
    ...
    def _reset_disc_color_candidate(self):
        self.disc_candidate_color = None
        self.disc_candidate_center = None
        self.disc_candidate_frames = 0
    ...
    def _disc_catch_ball_visible(self, detections):
        ...
        if not allowed:
            self._reset_disc_color_candidate()
            return None

        best = max(allowed, key=lambda det: float(det.get("area_percent") or 0.0))
        color = best.get("color")
        center = best.get("center")
        center_jump = 0.0
        if center is not None and self.disc_candidate_center is not None:
            center_jump = math.hypot(
                float(center[0]) - float(self.disc_candidate_center[0]),
                float(center[1]) - float(self.disc_candidate_center[1]),
            )
        same_candidate = (
            color == self.disc_candidate_color
            and (
                center is None
                or self.disc_candidate_center is None
                or center_jump <= DISC_CATCH_COLOR_JUMP_RESET_PX
            )
        )
        if same_candidate:
            self.disc_candidate_frames += 1
        else:
            self.disc_candidate_color = color
            self.disc_candidate_frames = 1
        self.disc_candidate_center = center

        if self.disc_candidate_frames < DISC_CATCH_COLOR_CONFIRM_FRAMES:
            print(
                "CHASSIS STATION DISC_CATCH confirming "
                f"{color} ball {self.disc_candidate_frames}/"
                f"{DISC_CATCH_COLOR_CONFIRM_FRAMES} "
    ...
    def update_chassis_station(
        ...
        if self.chassis_station_stage == "disc_detect":
            ...
            if self.disc_pulse_done:
                current_color = None if ball is None else ball.get("color")
                if (
                    ball is None
                    or current_color != self.disc_last_pulsed_color
                ):
                    self.disc_pulse_done = False
                    self.disc_last_pulsed_color = None
                    self._reset_disc_color_candidate()

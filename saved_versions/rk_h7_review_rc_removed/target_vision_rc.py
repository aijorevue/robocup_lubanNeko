...
class RedSquareGraspController:
    ...
    def _update_locked_grasp(self, frame_shape, now, can_preview_step):
        ...
        if self.algorithm_stage == "open":
            ...
            if can_command:
                ...
                if (
                    ...
                    self.algorithm_stage = "open_retry"
                    self.claw_settle_deadline = (
        ...
        if self.algorithm_stage == "open_retry":
            self.state = "retrying grasp claw open"
            if now < self.claw_settle_deadline:
                return f"waiting to retry grasp open {self.claw_settle_deadline - now:.1f}s"
            if can_command:
                self.id4 = self.id4_open
                self.open_attempts += 1
                result = self._send_claw(
                    "grasp claw open unconfirmed; conditional retry",
                    fault_on_failure=False,
                )
                if (
                    self.servo_bridge.last_command_ok
                    ...
                    return result
                if self.open_attempts < ID4_ACTION_MAX_ATTEMPTS:
                    self.claw_settle_deadline = time.monotonic() + CLAW_SETTLE_S
                    return (
                        f"{result}; grasp open attempt {self.open_attempts}/"
                        f"{ID4_ACTION_MAX_ATTEMPTS} unconfirmed, retry pending"
                    )
                if not self.servo_bridge.last_command_ok or self.servo_bridge.last_claw_assumed:
                    self.algorithm_stage = "fault"
                    self.state = "fault"
                    self.status = (
                        f"grasp claw open failed after {self.open_attempts} attempts; "
                        f"descend suppressed: {self.servo_bridge.status}"
                    )
                    print(f"ID4 FAULT {self.status}", flush=True)
                    self.arm_preview.publish(self.status)
                    return self.status
            return "waiting to retry grasp claw open"
        ...
        if self.algorithm_stage == "drop_open":
            ...
            if can_command:
                ...
                result = self._send_claw(
                    "carry pose reached; open claw to drop target",
                    fault_on_failure=False,
                )
                if (
                    self.servo_bridge.last_command_ok
                    ...
                    self.claw_settle_deadline = time.monotonic() + CLAW_SETTLE_S
                    return (
                        f"{result}; ID4 drop open unconfirmed, "
                        f"retrying up to {ID4_ACTION_MAX_ATTEMPTS} attempts"
                    )
                return result
        ...
        if self.algorithm_stage == "drop_open_retry":
            ...
            if can_command:
                ...
                if (
                    ...
                    self.claw_settle_deadline = time.monotonic() + CLAW_SETTLE_S
                    return result
                if self.drop_open_attempts < ID4_ACTION_MAX_ATTEMPTS:
                    self.claw_settle_deadline = time.monotonic() + CLAW_SETTLE_S
                    return (
                        f"{result}; drop open attempt {self.drop_open_attempts}/"
                        f"{ID4_ACTION_MAX_ATTEMPTS} unconfirmed, retry pending"
                    )
                if not self.servo_bridge.last_command_ok or self.servo_bridge.last_claw_assumed:
                    self.algorithm_stage = "fault"
                    self.state = "fault"
                    self.status = (
                        f"drop claw failed after {self.drop_open_attempts} attempts; "
                        f"arm held at drop pose: {self.servo_bridge.status}"
                    )
                    print(f"ID4 FAULT {self.status}", flush=True)
                    self.arm_preview.publish(self.status)
                    return self.status
        ...
        if self.algorithm_stage == "close_retry":
            ...
            if can_command:
                ...
                if (
                    ...
                    self.return_attempts = 0
                    return result
                if self.close_attempts < ID4_ACTION_MAX_ATTEMPTS:
                    self.claw_settle_deadline = time.monotonic() + CLAW_SETTLE_S
                    return (
                        f"{result}; grasp close attempt {self.close_attempts}/"
                        f"{ID4_ACTION_MAX_ATTEMPTS} unconfirmed, retry pending"
                    )
                if not self.servo_bridge.last_command_ok or self.servo_bridge.last_claw_assumed:
                    self.algorithm_stage = "fault"

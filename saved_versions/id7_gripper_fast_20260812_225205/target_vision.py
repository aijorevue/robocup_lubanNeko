...
DISC_CATCH_TARGET_TIMEOUT_S = 3.0
DISC_CATCH_GRIPPER_HALF_PERIOD_S = 0.25
...
class RedSquareGraspController:
    ...
    def _disc_catch_gripper_half_s(self):
        return max(0.08, DISC_CATCH_GRIPPER_HALF_PERIOD_S)
    ...
    def update_chassis_station(
        ...
        if self.chassis_station_stage == "disc_detect":
            ...
            self.status = (
                ...
                f"ID2={self.id2}; hold splitter ID4={splitter_target} then pulse ID7"
            )
        ...
        if self.chassis_station_stage == "disc_route_splitter":
            ...
            if splitter_target is None:
                ...
                if not self.servo_bridge.write_enabled:
                    ...
                    return f"preview DISC_CATCH splitter hold ID4={self.splitter_id4} before ID7 pulse"
                splitter_status = self._send_splitter_id4(
                    splitter_target,
                    "DISC_CATCH hold splitter before ID7 pulse",
                )
                if self.servo_bridge.last_command_ok:
                    self.chassis_station_stage = "disc_splitter_wait"
        ...
        if self.chassis_station_stage == "disc_open":
            ...
            if not self.servo_bridge.write_enabled:
                ...
                self.chassis_station_deadline = now + self._disc_catch_gripper_half_s()
                self.arm_preview.set_targets(self.id1, self.id2, self.id4, self.id6)
                return "preview DISC_CATCH open ID7 pulse"
            status = self._send_gripper_id7(self.id4_open, "DISC_CATCH open ID7 pulse")
            if self.servo_bridge.last_command_ok:
                self.chassis_station_stage = "disc_open_wait"
                self.chassis_station_deadline = time.monotonic() + self._disc_catch_gripper_half_s()
            return status
        ...
        if self.chassis_station_stage == "disc_close_wait":
            ...
            return "DISC_CATCH 500ms pulse complete; resume detecting until 3s no target"
...
def build_arg_parser():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="draw preview on the frame that produced the latest detection; "
        "default draws on the newest camera frame for lower visual latency",
    )
    ...
    try:
        while True:
            ...
            render_frame = (
                ...
                if args.display_detection_frame and detection_source_frame is not None
                else frame
            )

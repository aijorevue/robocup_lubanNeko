    ...
    def update_chassis_station(
        ...
        if self.chassis_station_stage == "disc_detect":
            ...
            if self.disc_pulse_done:
                ...
                if ball is None or current_color != self.disc_last_pulsed_color:
                    self.disc_pulse_done = False
                    self.disc_last_pulsed_color = None
                elif not self._disc_catch_ball_touches_edge(ball, frame_shape):
                    self.disc_pulse_done = False
                    self.disc_last_pulsed_color = None
                else:
                    remaining = max(0.0, self.chassis_station_no_target_deadline - now)
                    return f"DISC_CATCH pulse complete; waiting edge leave {remaining:.1f}s"
            if ball is None:
                remaining = max(0.0, self.chassis_station_no_target_deadline - now)
                return (
        ...
        if self.chassis_station_stage == "disc_close_wait":
            ...
            return "DISC_CATCH pulse complete; waiting next edge appearance"

...
BASE_YAW_TICKS_PER_REVOLUTION = 1000.0

ID1_ZERO_DEG_TICK = 460.0
ID1_NINETY_DEG_TICK = 80.0

ID2_NINETY_DEG_TICK = 250.0
ID2_FORWARD_180_DEG_TICK = 640.0

ID1_SAFE_LIMITS = (150, 710)
ID2_SAFE_LIMITS = (0, 769)
...
def id6_radians(tick):
    return (float(tick) - BASE_YAW_CENTER_TICK) * (
        2.0 * math.pi / BASE_YAW_TICKS_PER_REVOLUTION
    )


def id1_degrees(tick):
    physical_degrees = (ID1_ZERO_DEG_TICK - tick) * (
        90.0 / (ID1_ZERO_DEG_TICK - ID1_NINETY_DEG_TICK)
    )
    return 180.0 - physical_degrees


def id2_degrees(tick):
    return (ID2_FORWARD_180_DEG_TICK - tick) * (
        90.0 / (ID2_FORWARD_180_DEG_TICK - ID2_NINETY_DEG_TICK)
    )


def id2_tick_from_degrees(degrees):
    return ID2_FORWARD_180_DEG_TICK - degrees * (
        (ID2_FORWARD_180_DEG_TICK - ID2_NINETY_DEG_TICK) / 90.0
    )
...
def joint_positions(id1_tick, id2_tick, id4_tick, id6_tick=BASE_YAW_CENTER_TICK):
    orange = math.radians(id1_degrees(id1_tick))
    blue = math.radians(id2_degrees(id2_tick))
    gripper_range = GRIPPER_OPEN_TICK - GRIPPER_CLOSED_TICK
    gripper_ratio = max(
        0.0,
        min(1.0, (id4_tick - GRIPPER_CLOSED_TICK) / float(gripper_range)),
    )
    return [
        id6_radians(id6_tick),
        blue,
        orange - blue,
        orange,
        blue - orange,

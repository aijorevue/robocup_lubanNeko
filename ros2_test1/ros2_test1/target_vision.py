...
# Blue-field pulses are re-armed only after the completed close plus a full
# cooldown; target-center displacement is intentionally not used as a lock.
DISC_CATCH_BLUE_GRIPPER_COOLDOWN_S = 0.5
...
class TargetGraspController:
    ...
    def update_chassis_station(
        ...
        if self.chassis_station_stage == "disc_close_wait":
            ...
            if self.field_mode == FieldMode.BLUE:
                ...
                blue_cooldown = self.disc_last_pulsed_color == "blue"
                self.chassis_station_deadline = (
                    now + DISC_CATCH_BLUE_GRIPPER_COOLDOWN_S
                    if blue_cooldown
                    else max(now, self.disc_blue_channel_hold_deadline)
                )
                return (
                    "DISC_CATCH blue-field ID17 closed; "
                    + (
                        "blue-ball cooldown before next target "
                        f"for {DISC_CATCH_BLUE_GRIPPER_COOLDOWN_S:.2f}s"
                        if blue_cooldown
                        else "yellow-ball channel hold before returning to blue"
                    )
                )
        ...
        if self.chassis_station_stage == "disc_blue_channel_wait":
            ...
            blue_pulse = self.disc_last_pulsed_color == "blue"
            if blue_pulse:
                status = "DISC_CATCH blue cooldown complete; ID14 stays on blue channel"
            else:
                status = self._send_splitter_id4(
                    DISC_CATCH_SPLITTER_FIELD_TICK,
                    "DISC_CATCH blue-field ID14 return to blue after yellow",
                    splitter_time_ms=TASK1_ID14_TIME_MS,
                )
            if blue_pulse or self.servo_bridge.last_command_ok:
                self.disc_blue_channel_hold_deadline = 0.0
                self.disc_pulse_done = False
                self.disc_last_pulsed_color = None
                self.disc_last_pulsed_center = None
    ...
    def _update_column_catch_station(
        ...
        if self.chassis_station_stage == "column_ready":
            ...
            # The orbit is already running. Keep the arm high and wait for a
            # selected letter before asking H7 to stop.
            self.chassis_station_stage = "column_detect"
            self.column_target_armed = True
            return "COLUMN_CATCH detecting selected A/B/C/D letter during orbit"

        if self.chassis_station_stage == "column_detect":
            self.state = "COLUMN_CATCH detect selected letter"
            ...
            if letter_target is None:
                ...
                if self.column_target_absent_frames >= 3:
                    self.column_target_armed = True
                return "COLUMN_CATCH orbit detect; no selected letter"
            self.column_target_absent_frames = 0
            if not self.column_target_armed:
                return "COLUMN_CATCH waiting selected letter to leave before rearm"
            ...
            self.status = (
                f"COLUMN_CATCH letter {letter_target.get('letter')} detected; "
                "request H7 PAUSE"
            )
            print(f"CHASSIS STATION {self.status}", flush=True)
        ...
        if self.chassis_station_stage == "column_pause_request":
            ...
            if state == "PAUSED":
                ...
                return "COLUMN_CATCH H7 PAUSED; center target"
            if state == "STOPPED":
        ...
        if self.chassis_station_stage == "column_centering":
            ...
            if not detection_fresh or letter_target is None:
                self.visual_lost_frames += 1
                if self.visual_lost_frames > 8:
                    self.chassis_station_stage = "column_resume_abort"
                    self.status = "COLUMN_CATCH target lost while paused; resume H7"
                    return self.status
                return "COLUMN_CATCH paused; waiting fresh letter for centering"
            self.locked_target = self._copy_target(letter_target)
        ...
        if self.chassis_station_stage == "column_resume_abort":
            ...
            if chassis_link is None:
                return self._column_abort("NO_FORMAL_CHASSIS_LINK")
            if chassis_link.formal_column_pause_state() == "RESUMED":
                self.locked_target = None
                self.locked_plan = None
                self.chassis_station_stage = "column_detect"
                self.column_target_armed = False

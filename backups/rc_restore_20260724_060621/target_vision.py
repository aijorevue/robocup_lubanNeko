...
class RedSquareGraspController:
    ...
    def _visual_center_step(self, target, frame_shape, now, can_preview_step, label):
        ...
        return False, self._send_center_correction(
            ...
            f"dID2={self.id2 - previous_id2} dID6={self.id6 - previous_id6}",
            send_id2=send_id2,
            send_id6=send_id6,
        )
    ...
    def _id6_centering_delta(self, error_x_px):
        if abs(error_x_px) <= self.center_deadband_px:
            return 0
        delta = int(round(-error_x_px * self.id6_pixel_gain))
        delta = max(-self.id6_max_step_ticks, min(self.id6_max_step_ticks, delta))
        minimum_step = min(SINGLE_ID6_MIN_STEP_TICKS, self.id6_max_step_ticks)
        if abs(delta) < minimum_step:
    ...
    def _update_locked_grasp(self, frame_shape, now, can_preview_step, live_target=None):
        ...
        if self.algorithm_stage == "open_wait":
            ...
            self.algorithm_stage = self.next_stage_after_open or "descend"
            self.next_stage_after_open = None
        ...
        if self.algorithm_stage == "close":
            ...
            if not self.servo_bridge.write_enabled:
                ...
                self.algorithm_stage = "return"
                self.arm_preview.set_targets(self.id1, self.id2, self.id4, self.id6)
                return "preview claw closed; returning to standby"
            if can_command:
        ...
        if self.algorithm_stage == "release":
            ...
            if not self.servo_bridge.write_enabled:
                ...
                self.arm_preview.set_targets(self.id1, self.id2, self.id4, self.id6)
                return "preview release target; reclosing next"
            if can_command:
                result = self._send_id4(self.id4_open, "release target at standby")
                if self.servo_bridge.last_command_ok:
...
class ArmPreviewPublisher:
    ...
    def publish(self, detail, target=None, state=None):
        ...
        try:
            ...
            should_publish_joint = (
                joint_ticks != self.last_joint_ticks
                or now - self.last_joint_publish_time >= self.joint_publish_interval_s
            )

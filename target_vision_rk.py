        ...
        try:
            ...
            for command, expected_ack in commands:
                ...
                for attempt in range(2):
                    ...
                    response = self._read_text(0.25).strip()
...
class RedSquareGraspController:
    def __init__(
        ...
        self.startup_ready_timeout = time.monotonic() + 8.0
    ...
    def _startup_send_targets(self, id1, id2, id4, label):
        ...
        self._enforce_angle_gap()
        self.arm_preview.set_targets(self.id1, self.id2, self.id4)
        status = self.servo_bridge.send_targets(self.id1, self.id2, self.id4)
        self.last_command_time = time.monotonic()
        if not self.servo_bridge.last_command_ok:
            return self._startup_fault(f"{label} {status}")
        self.status = f"{label} ACK ID1={self.id1} ID2={self.id2} ID4={self.id4}"
        return self.status
    ...
    def _update_startup(self, now):
        ...
        if self.startup_stage == "wait_ready":
            ...
            if self._startup_positions_ready(feedback):
                ...
                id4_status = self.servo_bridge.send_targets(id4=self.id4)
                if not self.servo_bridge.last_command_ok:
                    return self._startup_fault(
                        f"startup existing preset ID4 {id4_status}"
                    )
                ...
                self.status = (
                    f"startup_ready existing preset ID1={self.id1} "
                    f"ID2={self.id2} ID4={self.id4} ACK"
                )
                return self.status
        ...
        if self.startup_stage == "verify_ready":
            ...
            if self._startup_positions_ready(feedback):
                ...
                return self.status
            if self.startup_feedback_attempts >= 3:
                return self._startup_fault(
                    f"ready feedback outside +/-{self.startup_position_tolerance}: "
                    f"{feedback or self.servo_bridge.status}"
                )
            return f"startup_verify retry {self.startup_feedback_attempts}/3"
    ...
    def _post_center_plan(self, target=None, frame_shape=None):
        ...
        return plan, (
            ...
            f"move x={planned_move_x_mm:.0f}/{move_x_mm:.0f}mm "
    ...
    def _update_locked_grasp(self, frame_shape, now, can_preview_step):
        ...
        if self.algorithm_stage == "open":
            ...
            if can_command:
                ...
                return self._send("locked target; open claw", require_feedback=False)
        ...
        if self.algorithm_stage == "approach":
            self.state = "locked target approach"
            if not self.servo_bridge.write_enabled:
                ...
                return (
                    f"preview algorithm approach ID1={self.id1}->{plan['id1']} "
            ...
            if can_command:
                ...
                return self._send(
                    f"locked target algorithm approach | {plan_text}",
                    require_feedback=True,
                )
            return "locked target; waiting algorithm approach"
        ...
        if self.algorithm_stage == "close":
            ...
            if not self.servo_bridge.write_enabled:
                ...
                self.arm_preview.set_targets(self.id1, self.id2, self.id4)
                return "preview algorithm grasp complete; claw closed"
            if can_command:
                self.id4 = self.id4_closed
                self.algorithm_stage = "complete"
                self.state = "claw_close_ack"
                return self._send("locked target; close claw", require_feedback=False)
    ...
    def update(self, target, frame_shape):
        ...
        if not centered:
            ...
            if not self.servo_bridge.write_enabled:
                preview_id1 = self._clamp(
                    self.id1 + delta_id1,
                    self.preview_id1_limits,
                )
                preview_id2 = self._clamp(
                    self.id2 + delta_id2,
                    self.preview_id2_limits,
                )

#!/usr/bin/env python3
"""Standalone task-three test: main-camera letter pickup while orbiting.

The H7 owns a closed-loop 450 mm radius, 360 degree orbit. RK owns the
main-camera ABCD detector and pauses the orbit only for a confirmed letter.
Arm motion reuses the task-two direct servo-board path and the measured IK
model from the deployed workspace.
"""

from __future__ import annotations

import argparse
import copy
import math
import os
import select
import sys
import time
from collections import deque

import cv2
import numpy as np


ROOT = os.path.dirname(os.path.abspath(__file__))
if ROOT not in sys.path:
    sys.path.insert(0, ROOT)

# Keep one implementation of the tested servo framing and arm behavior.
from task2_secondary_h7_test_app import (  # noqa: E402
    ARM_TIME_MS,
    CENTER_DEADBAND_PX,
    CENTER_ID2_RANGE,
    CENTER_ID6_RANGE,
    CENTER_STEP_TICKS,
    CENTER_TIME_MS,
    CENTER_TRACK_MAX_JUMP_PX,
    GRIPPER_CLOSED,
    GRIPPER_OPEN,
    GRIPPER_TIME_MS,
    HIGH,
    LETTER_WORK,
    LETTERS,
    MAIN_LETTER_MIN_CONFIDENCE,
    TARGET_TRACK_MISSING_TIMEOUT_S,
    HTD85_AUX_HIGH,
    HTD85_AUX_TIME_MS,
    ServoBoards,
    import_main_detector,
    letter_detections,
    open_camera,
)


MAIN_CAMERA = "/dev/v4l/by-path/platform-fc800000.usb-usb-0:1:1.0-video-index0"
H7_DEVICE = "/dev/h7_chassis"
ARM_DEVICE = "/dev/serial/by-id/usb-1a86_USB_Single_Serial_5C82109853-if00"

TARGET_VOTE_WINDOW = 10
# One valid observation inside the capture window is sufficient to stop H7.
# The orbit is already moving slowly near the center, and waiting for another
# frame can carry a target past the gripper axis.
TARGET_REQUIRED_VOTES = 1
# A moving target can shift substantially between two detector frames. Keep
# the main-camera detector strict, but do not reject a valid track because the
# chassis moved the target more than the task-two stationary tolerance.
TARGET_TRACK_MAX_JUMP_PX = max(320.0, CENTER_TRACK_MAX_JUMP_PX)
TARGET_RECHECK_DELAY_S = 0.15
ORBIT_RESCUE_INTERVAL_FRAMES = 3
ORBIT_RESCUE_MIN_CONFIDENCE = 38.0
TASK3_LETTERS = tuple(sorted(LETTERS))
TASK3_LETTER_SUCCESS_QUOTA = 2
TASK3_ORBIT_RADIUS_MM = 450
TASK3_ORBIT_ANGLE_DEG = 360
TASK3_LETTER_MIN_CONFIDENCE = max(40.0, MAIN_LETTER_MIN_CONFIDENCE - 5.0)
TASK3_HIGH = {1: 650, 2: 570, 6: 415}
# The pickup depth is relative to the final centered pose. Do not reuse the
# removed fixed task-two/task-three descent model here.
TASK3_DESCEND_OFFSET_TICKS = 100
# Candidates may be tracked while they cross the image, but a target is
# actionable only inside the center window below. This prevents a side target
# from pausing the orbit before it reaches the gripper axis.
TASK3_MAX_TRACK_CENTER_DISTANCE_PX = 360.0
TASK3_CAPTURE_CENTER_TOL_X_PX = 90.0
TASK3_CAPTURE_CENTER_TOL_Y_PX = 90.0
# Keep the single actionable observation inside the capture window; do not
# combine an old edge observation with a new centered observation.
TASK3_TARGET_VOTE_CENTER_TOL_PX = 90.0
# Keep the task-three-only rescue path from promoting weak background shapes to
# D. Task two and the shared detector thresholds remain unchanged.
TASK3_D_MIN_CONFIDENCE = 55.0

# Task three's station has a green surface, a white square, and a black
# Times New Roman glyph. These thresholds are task-three-only and leave the
# shared detector used by task two and the formal route untouched.
TASK3_GREEN_HUE_RANGE = (35, 105)
TASK3_GREEN_MIN_SATURATION = 40
TASK3_GREEN_MIN_VALUE = 25
TASK3_WHITE_MIN_VALUE = 190
TASK3_WHITE_MAX_SATURATION = 135
TASK3_WHITE_PATCH_MIN_AREA = 700.0
TASK3_WHITE_PATCH_MAX_AREA_RATIO = 0.12
TASK3_WHITE_PATCH_MIN_SIDE = 28.0
TASK3_WHITE_PATCH_MAX_SIDE_RATIO = 0.45
TASK3_WHITE_PATCH_MIN_RECTANGULARITY = 0.55
TASK3_WHITE_PATCH_MIN_SOLIDITY = 0.72
TASK3_WHITE_PATCH_MIN_GREEN_SUPPORT = 0.42
TASK3_WHITE_PATCH_MIN_GREEN_RING_SUPPORT = 0.55
TASK3_WHITE_PATCH_MIN_INNER_WHITE_RATIO = 0.12
TASK3_WHITE_PATCH_APPROX_EPSILON = 0.04
TASK3_WHITE_PATCH_MIN_APPROX_VERTICES = 4
TASK3_WHITE_PATCH_MAX_APPROX_VERTICES = 5
# A task-three station must be safely inside the camera view before it can
# pause the orbit. This rejects chassis parts clipped by the image boundary.
TASK3_WHITE_PATCH_EDGE_MARGIN_RATIO = 0.05
# A real white letter patch contains a dark glyph or other non-white interior;
# solid white hardware panels must never become orbit targets.
TASK3_WHITE_PATCH_MAX_INNER_WHITE_RATIO = 0.97
# A real square has four continuous white sides and white corner joins. A
# circular hole/reflection has white pixels only on scattered arcs.
TASK3_WHITE_PATCH_MIN_EDGE_WHITE_RATIO = 0.30
TASK3_WHITE_PATCH_MIN_CORNER_WHITE_RATIO = 0.18
TASK3_WHITE_PATCH_MIN_INNER_NONWHITE_RATIO = 0.15
TASK3_WHITE_PATCH_MAX_CIRCULARITY = 0.88
TASK3_WHITE_BLOCK_CONFIRM_FRAMES = 1
TASK3_WHITE_BLOCK_MATCH_CENTER_PX = 55.0
TASK3_WHITE_BLOCK_MATCH_IOU = 0.35
# During arm centering the locked block moves in the image. Keep associating
# it by bounded motion and size, without switching to another block.
TASK3_LOCKED_BLOCK_MAX_JUMP_PX = 220.0
TASK3_LOCKED_BLOCK_MAX_SIZE_RATIO = 2.5
# A handled block is only geometrically suppressed while it remains visible.
# After this many missing frames, a later candidate is allowed through again.
TASK3_HANDLED_BLOCK_REARM_FRAMES = 15
TASK3_LOCKED_LETTER_MAX_JUMP_PX = 260.0
TASK3_LOCKED_LETTER_MAX_SIZE_RATIO = 2.8
TASK3_GREEN_LETTER_MIN_CONFIDENCE = 0.42
TASK3_BLOCK_MAX_ANGLE_DEG = 14.0
# Keep H7 paused long enough for a fresh ROI-bound glyph result. A brief
# camera/detector gap must not turn a valid white block into an orbit skip.
TASK3_CLASSIFY_TIMEOUT_S = 3.0


class Task3H7Link:
    """Sequence-aware line transport for the isolated TASK3 protocol."""

    def __init__(self, path: str):
        self.path = path
        self.fd = None
        self.rx = bytearray()

    def open(self) -> None:
        if not os.path.exists(self.path):
            raise RuntimeError(f"H7 CDC missing: {self.path}")
        # Importing the tested UART setup avoids pyserial ownership changes.
        from task2_secondary_h7_test_app import open_uart

        self.fd = open_uart(self.path)
        print(f"TASK3 H7 OPEN {self.path}", flush=True)

    def send(self, line: str) -> None:
        if self.fd is None:
            self.open()
        payload = (line.rstrip("\r\n") + "\r\n").encode("ascii")
        offset = 0
        while offset < len(payload):
            offset += os.write(self.fd, payload[offset:])
        termios_drain(self.fd)
        print(f"TASK3 H7 TX {line.rstrip()}", flush=True)

    def _read_lines(self, timeout_s: float) -> list[str]:
        if self.fd is None:
            return []
        ready, _, _ = select.select([self.fd], [], [], timeout_s)
        if not ready:
            return []
        self.rx.extend(os.read(self.fd, 4096))
        lines = []
        while b"\n" in self.rx:
            raw, _, self.rx = self.rx.partition(b"\n")
            line = raw.decode("ascii", "replace").strip("\r")
            if line:
                print(f"TASK3 H7 RX {line}", flush=True)
                lines.append(line)
        return lines

    def poll(self) -> list[str]:
        """Read already available H7 lines without delaying camera capture."""
        return self._read_lines(0.0)

    @staticmethod
    def _matches(line: str, status: str, sequence: int) -> bool:
        fields = [item.strip() for item in line.split(",")]
        return (
            len(fields) >= 6
            and fields[:4] == ["H7", "TEST", "TASK3", status]
            and fields[4] == "SEQ"
            and fields[5] == str(sequence)
        )

    def wait_status(self, sequence: int, status: str, timeout_s: float) -> str:
        deadline = time.monotonic() + timeout_s
        while time.monotonic() < deadline:
            for line in self._read_lines(min(0.1, max(0.0, deadline - time.monotonic()))):
                if self._matches(line, status, sequence):
                    return line
                if self._matches(line, "ERR", sequence):
                    raise RuntimeError(f"H7_{status}_ERR {line}")
        raise RuntimeError(f"H7_{status}_TIMEOUT")

    def pause_until_confirmed(
        self, sequence: int, field: str, timeout_s: float
    ) -> str:
        """Retry PAUSE until H7 confirms that the motors are stopped.

        The ACK is useful diagnostics, but PAUSED is the safety point at which
        RK may move the arm. Retrying also covers a CDC packet lost while H7
        was transmitting an unrelated status line.
        """
        deadline = time.monotonic() + timeout_s
        next_send = 0.0
        command = f"RK,TEST,TASK3,PAUSE,SEQ,{sequence},FIELD,{field}"
        while time.monotonic() < deadline:
            now = time.monotonic()
            if now >= next_send:
                self.send(command)
                next_send = now + 0.25
            for line in self._read_lines(
                min(0.05, max(0.0, deadline - time.monotonic()))
            ):
                if self._matches(line, "PAUSED", sequence):
                    return line
                if self._matches(line, "ERR", sequence):
                    raise RuntimeError(f"H7_PAUSE_ERR {line}")
        raise RuntimeError(f"H7_PAUSED_TIMEOUT retries=PAUSE")

    def close(self) -> None:
        if self.fd is not None:
            os.close(self.fd)
            self.fd = None


def termios_drain(fd: int) -> None:
    """Drain a raw UART without importing termios at module import time."""
    import termios

    termios.tcdrain(fd)


def _motion_enhanced_frame(frame: np.ndarray) -> np.ndarray:
    """Recover local black-glyph contrast from short motion-blurred frames."""
    lab = cv2.cvtColor(frame, cv2.COLOR_BGR2LAB)
    lightness, a_channel, b_channel = cv2.split(lab)
    lightness = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8)).apply(
        lightness
    )
    enhanced = cv2.cvtColor(
        cv2.merge((lightness, a_channel, b_channel)), cv2.COLOR_LAB2BGR
    )
    blurred = cv2.GaussianBlur(enhanced, (0, 0), 1.2)
    return cv2.addWeighted(enhanced, 1.45, blurred, -0.45, 0.0)


def _configure_orbit_rescue_detector(detector):
    """Relax only the orbit rescue pass; task-two main detection stays strict."""
    for name, limit in (
        ("min_confidence", 0.38),
        ("dark_letter_min_confidence", 0.38),
    ):
        if hasattr(detector, name):
            try:
                setattr(detector, name, min(float(getattr(detector, name)), limit))
            except (TypeError, ValueError):
                pass
    return detector


def _classify_task3_white_patch(
    frame: np.ndarray, contour, detector, bbox: tuple[int, int, int, int]
) -> tuple[str | None, float, float]:
    """Classify the black glyph inside a white station patch.

    The generic detector searches the whole frame and can lose this target when
    the green station merges with the camera background. The task-three path
    first isolates the white patch, then gives the tight patch to the detector's
    black-glyph classifier. A rectified retry handles a rotated patch.
    """
    x, y, width, height = bbox
    candidates: list[tuple[str | None, float, float]] = []
    tight = frame[y : y + height, x : x + width]
    classify_dark = getattr(detector, "_classify_dark_letter", None)
    if callable(classify_dark) and tight.size:
        try:
            candidates.append(classify_dark(tight))
        except (cv2.error, TypeError, ValueError):
            pass

    rectify = getattr(detector, "_rectify", None)
    ordered_box = getattr(detector, "_ordered_box", None)
    if callable(rectify) and callable(ordered_box):
        try:
            rect = cv2.minAreaRect(contour)
            box = ordered_box(cv2.boxPoints(rect))
            rectified = rectify(frame, box, 160)
            if callable(classify_dark):
                candidates.append(classify_dark(rectified))
            classify_card = getattr(detector, "_classify", None)
            if callable(classify_card):
                candidates.append(classify_card(rectified))
        except (cv2.error, TypeError, ValueError):
            pass

    valid = [
        item for item in candidates
        if item[0] in TASK3_LETTERS
        and float(item[1]) >= TASK3_GREEN_LETTER_MIN_CONFIDENCE
        and 0.025 <= float(item[2]) <= 0.58
    ]
    if not valid:
        return None, 0.0, 0.0
    return max(valid, key=lambda item: float(item[1]))


def _task3_white_block_geometry(frame: np.ndarray) -> list[dict]:
    """Return white edged station blocks before attempting glyph recognition."""
    if frame is None or frame.ndim != 3:
        return []
    height, width = frame.shape[:2]
    frame_area = float(max(1, height * width))
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    green = cv2.inRange(
        hsv,
        np.array((TASK3_GREEN_HUE_RANGE[0], TASK3_GREEN_MIN_SATURATION,
                  TASK3_GREEN_MIN_VALUE), dtype=np.uint8),
        np.array((TASK3_GREEN_HUE_RANGE[1], 255, 255), dtype=np.uint8),
    )
    white = cv2.inRange(
        hsv,
        np.array((0, 0, TASK3_WHITE_MIN_VALUE), dtype=np.uint8),
        np.array((180, TASK3_WHITE_MAX_SATURATION, 255), dtype=np.uint8),
    )
    white = cv2.morphologyEx(
        white, cv2.MORPH_CLOSE,
        cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5)),
    )
    contours, _ = cv2.findContours(
        white, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
    )
    blocks = []
    for contour in sorted(contours, key=cv2.contourArea, reverse=True):
        area = float(cv2.contourArea(contour))
        if area < TASK3_WHITE_PATCH_MIN_AREA or area > frame_area * TASK3_WHITE_PATCH_MAX_AREA_RATIO:
            continue
        x, y, box_width, box_height = cv2.boundingRect(contour)
        edge_margin_x = int(round(width * TASK3_WHITE_PATCH_EDGE_MARGIN_RATIO))
        edge_margin_y = int(round(height * TASK3_WHITE_PATCH_EDGE_MARGIN_RATIO))
        if (
            x < edge_margin_x
            or y < edge_margin_y
            or x + box_width > width - edge_margin_x
            or y + box_height > height - edge_margin_y
        ):
            continue
        if min(box_width, box_height) < TASK3_WHITE_PATCH_MIN_SIDE:
            continue
        if max(box_width, box_height) > min(width, height) * TASK3_WHITE_PATCH_MAX_SIDE_RATIO:
            continue
        aspect = box_width / float(max(1, box_height))
        if not 0.55 <= aspect <= 1.80:
            continue
        rect = cv2.minAreaRect(contour)
        rect_width, rect_height = rect[1]
        rect_area = max(1.0, float(rect_width * rect_height))
        rectangularity = area / rect_area
        if rectangularity < TASK3_WHITE_PATCH_MIN_RECTANGULARITY:
            continue
        hull_area = max(1.0, float(cv2.contourArea(cv2.convexHull(contour))))
        if area / hull_area < TASK3_WHITE_PATCH_MIN_SOLIDITY:
            continue
        perimeter = float(cv2.arcLength(contour, True))
        approx_vertices = len(
            cv2.approxPolyDP(
                contour,
                TASK3_WHITE_PATCH_APPROX_EPSILON * perimeter,
                True,
            )
        )
        if not (
            TASK3_WHITE_PATCH_MIN_APPROX_VERTICES
            <= approx_vertices
            <= TASK3_WHITE_PATCH_MAX_APPROX_VERTICES
        ):
            continue
        inner_margin_x = max(4, int(round(box_width * 0.20)))
        inner_margin_y = max(4, int(round(box_height * 0.20)))
        inner = white[
            y + inner_margin_y:y + box_height - inner_margin_y,
            x + inner_margin_x:x + box_width - inner_margin_x,
        ]
        inner_white_ratio = cv2.countNonZero(inner) / float(max(1, inner.size))
        if not (
            TASK3_WHITE_PATCH_MIN_INNER_WHITE_RATIO
            <= inner_white_ratio
            <= TASK3_WHITE_PATCH_MAX_INNER_WHITE_RATIO
        ):
            continue
        edge_band = max(2, int(round(min(box_width, box_height) * 0.12)))
        edge_ratios = (
            cv2.countNonZero(white[y:y + edge_band, x:x + box_width])
            / float(max(1, edge_band * box_width)),
            cv2.countNonZero(
                white[y + box_height - edge_band:y + box_height,
                      x:x + box_width]
            ) / float(max(1, edge_band * box_width)),
            cv2.countNonZero(white[y:y + box_height, x:x + edge_band])
            / float(max(1, box_height * edge_band)),
            cv2.countNonZero(
                white[y:y + box_height,
                      x + box_width - edge_band:x + box_width]
            ) / float(max(1, box_height * edge_band)),
        )
        if min(edge_ratios) < TASK3_WHITE_PATCH_MIN_EDGE_WHITE_RATIO:
            continue
        corner_side = max(3, int(round(min(box_width, box_height) * 0.16)))
        corner_ratios = (
            cv2.countNonZero(white[y:y + corner_side, x:x + corner_side])
            / float(max(1, corner_side * corner_side)),
            cv2.countNonZero(
                white[y:y + corner_side,
                      x + box_width - corner_side:x + box_width]
            ) / float(max(1, corner_side * corner_side)),
            cv2.countNonZero(
                white[y + box_height - corner_side:y + box_height,
                      x:x + corner_side]
            ) / float(max(1, corner_side * corner_side)),
            cv2.countNonZero(
                white[y + box_height - corner_side:y + box_height,
                      x + box_width - corner_side:x + box_width]
            ) / float(max(1, corner_side * corner_side)),
        )
        if min(corner_ratios) < TASK3_WHITE_PATCH_MIN_CORNER_WHITE_RATIO:
            continue
        if 1.0 - inner_white_ratio < TASK3_WHITE_PATCH_MIN_INNER_NONWHITE_RATIO:
            continue
        perimeter = float(cv2.arcLength(contour, True))
        circularity = 4.0 * math.pi * area / float(max(1.0, perimeter * perimeter))
        if circularity > TASK3_WHITE_PATCH_MAX_CIRCULARITY:
            continue
        # minAreaRect may choose either edge as the reference for a nearly
        # square target. Normalize to the smallest signed edge tilt instead
        # of turning a real -9 degree block into an apparent 81 degree block.
        angle = float(rect[2])
        while angle <= -45.0:
            angle += 90.0
        while angle > 45.0:
            angle -= 90.0
        pad = max(10, int(round(max(box_width, box_height) * 0.18)))
        x0, y0 = max(0, x - pad), max(0, y - pad)
        x1, y1 = min(width, x + box_width + pad), min(height, y + box_height + pad)
        green_support = cv2.countNonZero(green[y0:y1, x0:x1]) / float(
            max(1, (y1 - y0) * (x1 - x0))
        )
        if green_support < TASK3_WHITE_PATCH_MIN_GREEN_SUPPORT:
            continue
        contour_mask = np.zeros_like(white)
        cv2.drawContours(contour_mask, [contour], -1, 255, -1)
        ring_kernel_size = max(
            7,
            int(round(max(box_width, box_height) * 0.16)),
        )
        if ring_kernel_size % 2 == 0:
            ring_kernel_size += 1
        expanded = cv2.dilate(
            contour_mask,
            cv2.getStructuringElement(
                cv2.MORPH_ELLIPSE,
                (ring_kernel_size, ring_kernel_size),
            ),
        )
        green_ring = cv2.bitwise_and(
            expanded,
            cv2.bitwise_not(contour_mask),
        )
        green_ring_pixels = green_ring > 0
        green_ring_support = cv2.countNonZero(
            green[green_ring_pixels]
        ) / float(max(1, int(np.count_nonzero(green_ring_pixels))))
        if green_ring_support < TASK3_WHITE_PATCH_MIN_GREEN_RING_SUPPORT:
            continue
        blocks.append({
            "kind": "column_block",
            "color": "white_edge",
            "source": "task3_white_edge_geometry",
            "center": (int(round(x + box_width / 2.0)),
                       int(round(y + box_height / 2.0))),
            "box": contour.astype(np.int32),
            "bbox": (int(x), int(y), int(box_width), int(box_height)),
            "projected_area": max(area, rect_area),
            "approx_vertices": approx_vertices,
            "inner_white_ratio": round(float(inner_white_ratio), 4),
            "green_ring_support": round(float(green_ring_support), 4),
            "fully_visible": x > 2 and y > 2 and x + box_width < width - 2
            and y + box_height < height - 2,
            "angle": round(angle, 1),
            "parallel": abs(angle) <= TASK3_BLOCK_MAX_ANGLE_DEG,
        })
    return blocks


def _task3_parallel_blocks(frame: np.ndarray, detector=None) -> list[dict]:
    blocks = [
        block for block in _task3_white_block_geometry(frame)
        if block.get("fully_visible", True) and block.get("parallel", False)
    ]
    if blocks or detector is None:
        return blocks
    try:
        letters = detector.detect(frame)
    except (AttributeError, cv2.error, TypeError, ValueError):
        return blocks
    inferred = [
        block for item in letters or []
        for block in [_task3_inferred_block_from_letter(frame, item)]
        if block is not None
    ]
    return inferred


def _task3_inferred_block_from_letter(frame: np.ndarray, letter: dict) -> dict | None:
    """Recover a white block whose glyph split its outer white contour."""
    if frame is None or letter.get("kind") != "letter":
        return None
    bbox = letter.get("bbox", ())
    if len(bbox) < 4 or not letter.get("fully_visible", True):
        return None
    height, width = frame.shape[:2]
    x, y, box_width, box_height = (int(value) for value in bbox[:4])
    if min(box_width, box_height) < TASK3_WHITE_PATCH_MIN_SIDE:
        return None
    if (
        x < int(width * TASK3_WHITE_PATCH_EDGE_MARGIN_RATIO)
        or y < int(height * TASK3_WHITE_PATCH_EDGE_MARGIN_RATIO)
        or x + box_width > width - int(width * TASK3_WHITE_PATCH_EDGE_MARGIN_RATIO)
        or y + box_height > height - int(height * TASK3_WHITE_PATCH_EDGE_MARGIN_RATIO)
    ):
        return None
    aspect = box_width / float(max(1, box_height))
    if not 0.65 <= aspect <= 1.55:
        return None
    angle = float(letter.get("angle", 90.0))
    if abs(angle) > TASK3_BLOCK_MAX_ANGLE_DEG:
        return None
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    white = cv2.inRange(
        hsv,
        np.array((0, 0, TASK3_WHITE_MIN_VALUE), dtype=np.uint8),
        np.array((180, TASK3_WHITE_MAX_SATURATION, 255), dtype=np.uint8),
    )
    green = cv2.inRange(
        hsv,
        np.array((TASK3_GREEN_HUE_RANGE[0], TASK3_GREEN_MIN_SATURATION, TASK3_GREEN_MIN_VALUE), dtype=np.uint8),
        np.array((TASK3_GREEN_HUE_RANGE[1], 255, 255), dtype=np.uint8),
    )
    patch = white[y:y + box_height, x:x + box_width]
    white_ratio = cv2.countNonZero(patch) / float(max(1, patch.size))
    if white_ratio < 0.30:
        return None
    edge_band = max(2, int(round(min(box_width, box_height) * 0.12)))
    edge_ratios = (
        cv2.countNonZero(white[y:y + edge_band, x:x + box_width]) / float(max(1, edge_band * box_width)),
        cv2.countNonZero(white[y + box_height - edge_band:y + box_height, x:x + box_width]) / float(max(1, edge_band * box_width)),
        cv2.countNonZero(white[y:y + box_height, x:x + edge_band]) / float(max(1, box_height * edge_band)),
        cv2.countNonZero(white[y:y + box_height, x + box_width - edge_band:x + box_width]) / float(max(1, box_height * edge_band)),
    )
    if min(edge_ratios) < 0.25:
        return None
    inner_margin_x = max(4, int(round(box_width * 0.16)))
    inner_margin_y = max(4, int(round(box_height * 0.16)))
    inner = white[y + inner_margin_y:y + box_height - inner_margin_y, x + inner_margin_x:x + box_width - inner_margin_x]
    inner_white_ratio = cv2.countNonZero(inner) / float(max(1, inner.size))
    if not 0.30 <= inner_white_ratio <= 0.98:
        return None
    mask = np.zeros_like(white)
    cv2.rectangle(mask, (x, y), (x + box_width, y + box_height), 255, -1)
    kernel_size = max(7, int(round(max(box_width, box_height) * 0.16)))
    if kernel_size % 2 == 0:
        kernel_size += 1
    expanded = cv2.dilate(mask, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (kernel_size, kernel_size)))
    ring = cv2.bitwise_and(expanded, cv2.bitwise_not(mask))
    ring_pixels = ring > 0
    green_ring_support = cv2.countNonZero(green[ring_pixels]) / float(max(1, int(np.count_nonzero(ring_pixels))))
    if green_ring_support < 0.30:
        return None
    contour_box = np.asarray(letter.get("box", ()), dtype=np.int32)
    if contour_box.size != 8:
        contour_box = np.asarray(((x, y), (x + box_width, y), (x + box_width, y + box_height), (x, y + box_height)), dtype=np.int32)
    return {
        "kind": "column_block",
        "color": "white_edge",
        "source": "column_letter_box_inferred_geometry",
        "center": tuple(map(int, letter.get("center", (x + box_width // 2, y + box_height // 2)))),
        "box": contour_box.reshape(4, 2),
        "bbox": (x, y, box_width, box_height),
        "projected_area": float(box_width * box_height),
        "inner_white_ratio": round(float(inner_white_ratio), 4),
        "green_ring_support": round(float(green_ring_support), 4),
        "fully_visible": True,
        "angle": round(angle, 1),
        "parallel": True,
    }


def _task3_bbox_iou(first, second) -> float:
    if not first or not second or len(first) < 4 or len(second) < 4:
        return 0.0
    ax, ay, aw, ah = (float(value) for value in first[:4])
    bx, by, bw, bh = (float(value) for value in second[:4])
    ax2, ay2 = ax + max(0.0, aw), ay + max(0.0, ah)
    bx2, by2 = bx + max(0.0, bw), by + max(0.0, bh)
    intersection = max(0.0, min(ax2, bx2) - max(ax, bx)) * max(
        0.0, min(ay2, by2) - max(ay, by)
    )
    union = max(1.0, aw * ah + bw * bh - intersection)
    return intersection / union


def _task3_blocks_match(first: dict | None, second: dict | None) -> bool:
    if first is None or second is None:
        return False
    first_center = np.asarray(first.get("center", ()), dtype=np.float32)
    second_center = np.asarray(second.get("center", ()), dtype=np.float32)
    if first_center.shape != (2,) or second_center.shape != (2,):
        return False
    return (
        float(np.linalg.norm(first_center - second_center))
        <= TASK3_WHITE_BLOCK_MATCH_CENTER_PX
        and _task3_bbox_iou(first.get("bbox"), second.get("bbox"))
        >= TASK3_WHITE_BLOCK_MATCH_IOU
    )


def _task3_block_tracks_match(first: dict | None, second: dict | None) -> bool:
    """Match the same block after an arm-centering movement."""
    if first is None or second is None:
        return False
    first_center = np.asarray(first.get("center", ()), dtype=np.float32)
    second_center = np.asarray(second.get("center", ()), dtype=np.float32)
    first_bbox = first.get("bbox", ())
    second_bbox = second.get("bbox", ())
    if (
        first_center.shape != (2,)
        or second_center.shape != (2,)
        or len(first_bbox) < 4
        or len(second_bbox) < 4
    ):
        return False
    first_size = max(float(first_bbox[2]), float(first_bbox[3]), 1.0)
    second_size = max(float(second_bbox[2]), float(second_bbox[3]), 1.0)
    size_ratio = max(first_size, second_size) / min(first_size, second_size)
    return (
        float(np.linalg.norm(first_center - second_center))
        <= TASK3_LOCKED_BLOCK_MAX_JUMP_PX
        and size_ratio <= TASK3_LOCKED_BLOCK_MAX_SIZE_RATIO
    )


def _task3_candidate_block(candidate: dict) -> dict:
    """Return the geometric parent block attached to a letter detection."""
    return {
        "bbox": tuple(candidate.get("task3_block_bbox", candidate.get("bbox", ()))),
        "center": tuple(candidate.get("task3_block_center", candidate.get("center", ()))),
    }


def _task3_record_handled_block(
    handled_blocks: list[dict], label: str, block: dict | None
) -> None:
    """Lock a processed block while it remains visible in the orbit view."""
    if block is None:
        return
    candidate = {
        "label": str(label).upper(),
        "block": {
            "bbox": tuple(block.get("bbox", ())),
            "center": tuple(block.get("center", ())),
        },
    }
    if len(candidate["block"]["bbox"]) < 4:
        return
    if np.asarray(candidate["block"]["center"], dtype=np.float32).shape != (2,):
        return
    if any(
        _task3_block_tracks_match(existing.get("block"), candidate["block"])
        for existing in handled_blocks
    ):
        return
    handled_blocks.append(candidate)
    print(
        f"TASK3 BLOCK_LOCKED label={candidate['label']} "
        f"center={candidate['block']['center']} action=SKIP_WHILE_VISIBLE",
        flush=True,
    )


def _task3_has_grabbed_letter(
    frame: np.ndarray, detector, block: dict, grabbed: dict[str, int]
) -> bool:
    """Recognize an already handled glyph before pausing the orbit again."""
    if not grabbed:
        return False
    try:
        detections = _task3_green_white_letter_detections(frame, detector)
    except (AttributeError, cv2.error, TypeError, ValueError):
        return False
    for candidate in detections:
        label = str(candidate.get("letter", "")).upper()
        if (
            grabbed.get(label, 0) >= TASK3_LETTER_SUCCESS_QUOTA
            and _task3_block_tracks_match(block, _task3_candidate_block(candidate))
        ):
            return True
    return False


def _task3_bounded_letter_fallback(
    frame: np.ndarray,
    detector,
    label: str,
    last_target: dict,
    locked_block: dict,
) -> list[dict]:
    """Recover a locked glyph if motion temporarily hides its white border.

    This is intentionally a bounded track, not a whole-frame fallback: the
    label, previous center, and approximate glyph size must all match the
    already classified block.
    """
    if detector is None:
        return []
    previous_center = np.asarray(last_target.get("center", ()), dtype=np.float32)
    if previous_center.shape != (2,):
        return []
    previous_bbox = last_target.get("bbox", ())
    previous_size = (
        max(float(previous_bbox[2]), float(previous_bbox[3]), 1.0)
        if len(previous_bbox) >= 4 else None
    )
    block_center = np.asarray(locked_block.get("center", ()), dtype=np.float32)
    block_bbox = locked_block.get("bbox", ())
    block_size = (
        max(float(block_bbox[2]), float(block_bbox[3]), 1.0)
        if len(block_bbox) >= 4 else 120.0
    )
    try:
        raw = detector.detect(frame)
    except (AttributeError, cv2.error, TypeError, ValueError):
        return []
    candidates = []
    for item in raw or []:
        if (
            item.get("kind") != "letter"
            or str(item.get("letter", "")).upper() != label
            or not item.get("fully_visible", True)
        ):
            continue
        center = np.asarray(item.get("center", ()), dtype=np.float32)
        if center.shape != (2,):
            continue
        if float(np.linalg.norm(center - previous_center)) > TASK3_LOCKED_LETTER_MAX_JUMP_PX:
            continue
        if block_center.shape == (2,) and float(np.linalg.norm(center - block_center)) > max(
            TASK3_LOCKED_BLOCK_MAX_JUMP_PX * 2.0, block_size * 3.0
        ):
            continue
        bbox = item.get("bbox", ())
        if previous_size is not None and len(bbox) >= 4:
            item_size = max(float(bbox[2]), float(bbox[3]), 1.0)
            if max(previous_size, item_size) / min(previous_size, item_size) > TASK3_LOCKED_LETTER_MAX_SIZE_RATIO:
                continue
        recovered = dict(item)
        recovered["task3_block_bbox"] = tuple(block_bbox)
        recovered["task3_block_center"] = tuple(center)
        candidates.append(recovered)
    return sorted(
        candidates,
        key=lambda item: float(
            np.linalg.norm(
                np.asarray(item.get("center"), dtype=np.float32) - previous_center
            )
        ),
    )


def _task3_candidate_in_block(candidate: dict, block: dict) -> bool:
    """Associate a glyph with the exact white block that paused the orbit."""
    center = np.asarray(candidate.get("center", ()), dtype=np.float32)
    bbox = block.get("bbox", ())
    if center.shape != (2,) or len(bbox) < 4:
        return False
    x, y, width, height = (float(value) for value in bbox[:4])
    pad_x = max(12.0, width * 0.22)
    pad_y = max(12.0, height * 0.22)
    return x - pad_x <= float(center[0]) <= x + width + pad_x and (
        y - pad_y <= float(center[1]) <= y + height + pad_y
    )


def _task3_green_white_letter_detections(
    frame: np.ndarray, detector
) -> list[dict]:
    """Find letters on white patches supported by the green station surface."""
    if frame is None or frame.ndim != 3:
        return []
    height, width = frame.shape[:2]
    detections: list[dict] = []
    for block in _task3_white_block_geometry(frame):
        x, y, box_width, box_height = block["bbox"]
        contour = block["box"]
        letter, confidence, occupancy = _classify_task3_white_patch(
            frame, contour, detector, (x, y, box_width, box_height)
        )
        if letter is None:
            continue
        detections.append(
            {
                "kind": "letter",
                "letter": letter,
                "color": "green_station_white_patch",
                "source": "task3_green_white_black_letter",
                "confidence": round(float(confidence) * 100.0, 1),
                "glyph_occupancy": round(float(occupancy), 4),
                "center": block["center"],
                "box": contour.astype(np.int32),
                "bbox": (int(x), int(y), int(box_width), int(box_height)),
                "projected_area": block["projected_area"],
                "fully_visible": block["fully_visible"],
                "angle": block["angle"],
                "distance_cm": None,
                "task3_block_bbox": (int(x), int(y), int(box_width), int(box_height)),
                "task3_block_center": tuple(block["center"]),
            }
        )
    if detections:
        return detections
    # The glyph may split the white border into disconnected regions. In that
    # case use the already validated letter box to infer the local parent
    # block, while retaining the same geometric gates before association.
    try:
        raw_letters = detector.detect(frame)
    except (AttributeError, cv2.error, TypeError, ValueError):
        raw_letters = []
    for item in raw_letters or []:
        label = str(item.get("letter", "")).upper()
        if item.get("kind") != "letter" or label not in TASK3_LETTERS:
            continue
        block = _task3_inferred_block_from_letter(frame, item)
        if block is None:
            continue
        inferred = dict(item)
        inferred.update(
            {
                "source": "task3_letter_box_inferred",
                "color": "green_station_white_patch",
                "task3_block_bbox": tuple(block["bbox"]),
                "task3_block_center": tuple(block["center"]),
                "distance_cm": item.get("distance_cm"),
            }
        )
        detections.append(inferred)
    return detections


def _merge_letter_candidates(candidates: list[dict]) -> list[dict]:
    """Keep the strongest observation of each nearby letter candidate."""
    merged = []
    for candidate in sorted(
        candidates,
        key=lambda item: float(item.get("confidence", 0.0)),
        reverse=True,
    ):
        label = str(candidate.get("letter", "")).upper()
        center = np.asarray(candidate.get("center", (0, 0)), dtype=np.float32)
        duplicate = any(
            label == str(existing.get("letter", "")).upper()
            and np.linalg.norm(
                center - np.asarray(existing.get("center", (0, 0)), dtype=np.float32)
            ) <= 0.35 * max(
                float(candidate.get("bbox", (0, 0, 1, 1))[2]),
                float(candidate.get("bbox", (0, 0, 1, 1))[3]),
                float(existing.get("bbox", (0, 0, 1, 1))[2]),
                float(existing.get("bbox", (0, 0, 1, 1))[3]),
            )
            for existing in merged
        )
        if not duplicate:
            merged.append(candidate)
    return merged


def _letter_candidates(
    frame: np.ndarray,
    detector,
    grabbed: dict[str, int],
    min_confidence: float,
    rescue_detector=None,
    frame_index: int = 0,
) -> list[dict]:
    # The task-three station geometry is authoritative. Never fall back to a
    # whole-frame letter detector here: a letter on another object can otherwise
    # be promoted after a white hardware edge or reflection triggers a pause.
    station_detections = _task3_green_white_letter_detections(frame, detector)
    detections = station_detections
    height, width = frame.shape[:2]
    frame_center = np.asarray((width / 2.0, height / 2.0), dtype=np.float32)

    def task3_actionable(item: dict) -> bool:
        center = np.asarray(item.get("center", frame_center), dtype=np.float32)
        center_distance = float(np.linalg.norm(center - frame_center))
        if center_distance > TASK3_MAX_TRACK_CENTER_DISTANCE_PX:
            return False
        label = str(item.get("letter", "")).upper()
        if label == "D" and float(item.get("confidence", 0.0)) < TASK3_D_MIN_CONFIDENCE:
            return False
        return True

    return [
        item for item in detections
        if str(item.get("letter", "")).upper() in TASK3_LETTERS
        and grabbed.get(str(item.get("letter", "")).upper(), 0)
        < TASK3_LETTER_SUCCESS_QUOTA
        and task3_actionable(item)
    ]


def _task3_classify_paused_block(
    camera: cv2.VideoCapture,
    detector,
    rescue_detector,
    block: dict,
    grabbed: dict[str, int],
    timeout_s: float,
) -> dict | None:
    """Classify the block only after H7 has confirmed PAUSED."""
    deadline = time.monotonic() + timeout_s
    block_center = np.asarray(block.get("center", (400, 300)), dtype=np.float32)
    locked_block = {
        "bbox": tuple(block.get("bbox", ())),
        "center": tuple(block.get("center", block_center)),
    }
    block_size = max(
        float(value) for value in block.get("bbox", (0, 0, 80, 80))[2:]
    )
    max_distance = max(120.0, block_size * 1.6)
    frame_index = 0
    while time.monotonic() < deadline:
        ok, frame = camera.read()
        if not ok or frame is None:
            time.sleep(0.01)
            continue
        candidates = _task3_green_white_letter_detections(frame, detector)
        frame_index += 1
        associated = []
        for candidate in candidates:
            label = str(candidate.get("letter", "")).upper()
            if label not in TASK3_LETTERS:
                continue
            center = np.asarray(candidate.get("center", ()), dtype=np.float32)
            if center.shape != (2,):
                continue
            candidate_block = _task3_candidate_block(candidate)
            if (
                np.linalg.norm(center - block_center) <= max_distance
                and _task3_block_tracks_match(locked_block, candidate_block)
            ):
                associated.append(candidate)
        if not associated:
            # The white border can disappear for a frame after PAUSED. Use
            # the already locked block and center as the only rescue scope;
            # never promote an unrelated full-frame letter.
            try:
                raw_letters = detector.detect(frame)
            except (AttributeError, cv2.error, TypeError, ValueError):
                raw_letters = []
            for candidate in raw_letters or []:
                label = str(candidate.get("letter", "")).upper()
                center = np.asarray(candidate.get("center", ()), dtype=np.float32)
                if (
                    candidate.get("kind") == "letter"
                    and label in TASK3_LETTERS
                    and center.shape == (2,)
                    and np.linalg.norm(center - block_center) <= max_distance
                    and _task3_candidate_in_block(candidate, block)
                ):
                    associated.append(candidate)
        if associated:
            target = min(
                associated,
                key=lambda item: float(
                    np.linalg.norm(
                        np.asarray(item.get("center", block_center), dtype=np.float32)
                        - block_center
                    )
                ),
            )
            target = dict(target)
            target_block = _task3_candidate_block(target)
            target["task3_block_bbox"] = tuple(target_block.get("bbox", ()))
            target["task3_block_center"] = tuple(target_block.get("center", block_center))
            if target.get("distance_cm") is None:
                try:
                    raw = [
                        item for item in detector.detect(frame)
                        if str(item.get("letter", "")).upper()
                        == str(target.get("letter", "")).upper()
                        and item.get("distance_cm") is not None
                        and _task3_candidate_in_block(item, target_block)
                    ]
                    if raw:
                        target["distance_cm"] = min(
                            raw,
                            key=lambda item: float(
                                np.linalg.norm(
                                    np.asarray(item.get("center", block_center), dtype=np.float32)
                                    - block_center
                                )
                            ),
                        ).get("distance_cm")
                except (AttributeError, cv2.error, TypeError, ValueError):
                    pass
            target["frame_shape"] = frame.shape
            target["task3_classified_after_pause"] = True
            return target
        time.sleep(0.01)
    return None


def _task3_capture_authorized(
    target: dict | None, block: dict, grabbed: dict[str, int]
) -> bool:
    """Permit arm descent only for the paused block's selected letter."""
    if not isinstance(target, dict) or not target.get("task3_classified_after_pause"):
        return False
    label = str(target.get("letter", "")).upper()
    if (
        label not in TASK3_LETTERS
        or grabbed.get(label, 0) >= TASK3_LETTER_SUCCESS_QUOTA
    ):
        return False
    if not _task3_candidate_in_block(target, block):
        return False
    return _task3_block_tracks_match(block, _task3_candidate_block(target))


def _nearest_target(candidates: list[dict], frame_shape) -> dict | None:
    if not candidates:
        return None
    height, width = frame_shape[:2]
    center = np.asarray((width / 2.0, height / 2.0), dtype=np.float32)
    return min(
        candidates,
        key=lambda item: float(
            np.linalg.norm(
                np.asarray(item.get("center", center), dtype=np.float32) - center
            )
        ),
    )


def _draw_view(frame: np.ndarray, candidates: list[dict], grabbed: dict[str, int], state: str):
    view = frame.copy()
    for item in candidates:
        x, y = map(int, item.get("center", (0, 0)))
        label = str(item.get("letter", "?")).upper()
        cv2.circle(view, (x, y), 8, (0, 255, 0), 2)
        cv2.putText(
            view, label, (x + 10, y), cv2.FONT_HERSHEY_SIMPLEX,
            0.8, (0, 255, 0), 2,
        )
    cv2.putText(
        view,
        f"TASK3 MAIN / {state} / GRABBED="
        f"{','.join(f'{key}:{value}' for key, value in sorted(grabbed.items())) or '-'}",
        (15, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.62, (255, 255, 255), 2,
    )
    return view


def _stable_target(
    candidates: list[dict],
    frame_shape,
    history: deque,
) -> dict | None:
    target = _nearest_target(candidates, frame_shape)
    if target is None:
        # An edge target that disappears must not leave enough stale votes to
        # pause and grab it later. The orbit keeps running and starts a fresh
        # confirmation when another target reaches the center window.
        history.clear()
        return None

    key = str(target.get("letter", "")).upper()
    center = tuple(map(int, target.get("center", (0, 0))))
    height, width = frame_shape[:2]
    error_x = float(center[0]) - width / 2.0
    error_y = float(center[1]) - height / 2.0
    if (
        abs(error_x) > TASK3_CAPTURE_CENTER_TOL_X_PX
        or abs(error_y) > TASK3_CAPTURE_CENTER_TOL_Y_PX
    ):
        # Recognition while moving is only a track hint. Do not pause until
        # the same letter is actually in the gripper's central capture area.
        history.clear()
        return None
    # Require genuinely consecutive observations of the same target. A
    # filtered vote over an old deque could otherwise confirm a target after
    # another letter had already crossed the capture window.
    if history:
        previous_key, previous_center, _ = history[-1]
        if (
            previous_key != key
            or previous_center is None
            or np.linalg.norm(
                np.asarray(previous_center, dtype=np.float32)
                - np.asarray(center, dtype=np.float32)
            ) > TASK3_TARGET_VOTE_CENTER_TOL_PX
        ):
            history.clear()
    history.append((key, center, dict(target)))
    if len(history) < TARGET_REQUIRED_VOTES:
        return None
    return max((item for item_key, item_center, item in history),
               key=lambda item: float(item.get("confidence", 0.0)))


def _letter_grasp(boards: ServoBoards, target: dict, grasp_model) -> None:
    """Execute the task-three letter grasp sequence at a paused orbit station."""
    distance_cm = target.get("distance_cm")
    if distance_cm is None:
        raise RuntimeError("TARGET_DEPTH_INVALID")
    try:
        solved_id1, solved_id2 = grasp_model(float(distance_cm))
    except (TypeError, ValueError) as exc:
        raise RuntimeError(f"TARGET_DEPTH_INVALID {exc}") from exc
    boards.open_gripper()
    centered_id1 = int(target.get("center_id1", TASK3_HIGH[1]))
    centered_id2 = int(target.get("center_id2", TASK3_HIGH[2]))
    centered_id6 = int(target.get("center_id6", TASK3_HIGH[6]))
    retreat_id2 = max(CENTER_ID2_RANGE[0], centered_id2 - TASK3_DESCEND_OFFSET_TICKS)
    boards.arm({2: retreat_id2}, ARM_TIME_MS)
    print(
        f"TASK3 POST_OPEN_ID2_RETREAT ID2={centered_id2}->{retreat_id2} "
        f"DELTA=-{TASK3_DESCEND_OFFSET_TICKS}",
        flush=True,
    )
    time.sleep(ARM_TIME_MS / 1000.0)
    descend = {"id1": int(solved_id1), "id2": int(solved_id2)}
    boards.arm(
        {
            1: descend["id1"],
            2: descend["id2"],
            6: centered_id6,
        }
    )
    print(
        f"TASK3 LETTER_DESCEND letter={target['letter']} "
        f"CENTER_ID1={centered_id1} CENTER_ID2={centered_id2} "
        f"ID1={descend['id1']} ID2={descend['id2']} "
        f"ID6={centered_id6} depth_cm={float(distance_cm):.1f} model=measured_7_30cm",
        flush=True,
    )
    time.sleep(ARM_TIME_MS / 1000.0)
    boards.close_gripper()
    _task3_pose_high(boards)
    boards.arm(LETTER_WORK)
    time.sleep(ARM_TIME_MS / 1000.0)
    boards.open_gripper()
    boards.close_gripper()
    _task3_pose_high(boards)
    print(
        "TASK3 LETTER_DONE "
        f"letter={target['letter']} ID1={LETTER_WORK[1]} "
        f"ID2={LETTER_WORK[2]} ID6={LETTER_WORK[6]} "
        f"GRIPPER={GRIPPER_OPEN}->{GRIPPER_CLOSED}",
        flush=True,
    )


def _task3_pose_high(boards: ServoBoards) -> None:
    """Use task-three's independent high pose without changing task two."""
    boards.arm(TASK3_HIGH)
    boards.aux_high()
    time.sleep(max(ARM_TIME_MS, HTD85_AUX_TIME_MS) / 1000.0)


def _task3_stop_safely(h7: Task3H7Link, sequence: int, field: str, boards=None):
    """Return the arm high before stopping H7 after a control failure."""
    if boards is not None:
        try:
            _task3_pose_high(boards)
            print("TASK3 SAFE_RECOVERY arm=HIGH", flush=True)
        except Exception as exc:
            print(f"TASK3 SAFE_RECOVERY arm=HIGH_FAILED error={exc}", flush=True)
    try:
        h7.send(f"RK,TEST,TASK3,STOP,SEQ,{sequence},FIELD,{field}")
        h7.wait_status(sequence, "STOPPED", 2.0)
    except Exception as exc:
        print(f"TASK3 SAFE_RECOVERY h7=STOP_FAILED error={exc}", flush=True)


def _task3_resume_or_recover(
    h7: Task3H7Link,
    sequence: int,
    field: str,
    timeout_s: float,
    boards: ServoBoards,
    reason: str,
):
    """Resume H7, or retract the arm and stop safely if RESUMED times out."""
    try:
        h7.send(f"RK,TEST,TASK3,RESUME,SEQ,{sequence},FIELD,{field}")
        return h7.wait_status(sequence, "RESUMED", timeout_s)
    except Exception as exc:
        print(
            f"TASK3 RESUME_FAILED reason={reason} error={exc}; "
            "action=SAFE_HIGH_THEN_STOP",
            flush=True,
        )
        _task3_stop_safely(h7, sequence, field, boards)
        raise


def _import_grasp_model():
    """Load the same measured 7-30 cm calibration used by formal task three."""
    sys.path.insert(0, str(os.path.join(ROOT, "ros2_test1")))
    from ros2_test1.grasp_calibration import calibrated_grasp_ticks
    return calibrated_grasp_ticks


def _center_task3_target(
    camera: cv2.VideoCapture,
    detector,
    rescue_detector,
    reference: dict,
    boards: ServoBoards,
) -> dict:
    """Re-center a paused target with the same rescue path used in orbit."""
    id1 = int(TASK3_HIGH[1])
    id2 = int(TASK3_HIGH[2])
    id6 = int(TASK3_HIGH[6])
    corrections = 0
    frame_index = 0
    last_target = reference
    missing_started = None
    reference_label = str(reference.get("letter", "")).upper()
    locked_block = {
        "bbox": tuple(reference.get("task3_block_bbox", reference.get("bbox", ()))),
        "center": tuple(reference.get("task3_block_center", reference.get("center", ()))),
    }
    last_valid_distance = reference.get("distance_cm")
    try:
        if last_valid_distance is not None:
            last_valid_distance = float(last_valid_distance)
    except (TypeError, ValueError):
        last_valid_distance = None

    while True:
        ok, frame = camera.read()
        if not ok or frame is None:
            time.sleep(0.01)
            continue
        candidates = [
            item for item in _letter_candidates(
                frame,
                detector,
                set(),
                TASK3_LETTER_MIN_CONFIDENCE,
                rescue_detector,
                frame_index,
            )
            if str(item.get("letter", "")).upper() == reference_label
            and _task3_block_tracks_match(
                locked_block,
                _task3_candidate_block(item),
            )
        ]
        if not candidates:
            candidates = _task3_bounded_letter_fallback(
                frame, detector, reference_label, last_target, locked_block
            )
        frame_index += 1
        if not candidates:
            now = time.monotonic()
            if missing_started is None:
                missing_started = now
            if now - missing_started >= TARGET_TRACK_MISSING_TIMEOUT_S:
                print(
                    "TASK3 TARGET TRACK_TIMEOUT "
                    f"missing_s={now - missing_started:.1f}",
                    flush=True,
                )
                raise RuntimeError("TARGET_TRACK_TIMEOUT")
            continue

        missing_started = None
        previous_center = np.asarray(
            last_target.get("center", ()), dtype=np.float32
        )
        if previous_center.shape == (2,):
            distances = [
                float(
                    np.linalg.norm(
                        np.asarray(item.get("center", previous_center), dtype=np.float32)
                        - previous_center
                    )
                )
                for item in candidates
            ]
            nearest_index = int(np.argmin(distances))
            target = (
                candidates[nearest_index]
                if distances[nearest_index] <= TARGET_TRACK_MAX_JUMP_PX
                else max(
                    candidates,
                    key=lambda item: float(item.get("confidence", 0.0)),
                )
            )
        else:
            target = max(
                candidates,
                key=lambda item: float(item.get("confidence", 0.0)),
            )
        target_block = _task3_candidate_block(target)
        locked_block = target_block
        target["task3_block_bbox"] = tuple(target_block.get("bbox", ()))
        target["task3_block_center"] = tuple(target_block.get("center", ()))
        last_target = target

        height, width = frame.shape[:2]
        cx, cy = target.get("center", (width / 2.0, height / 2.0))
        error_x = float(cx) - width / 2.0
        error_y = float(cy) - height / 2.0
        if abs(error_x) <= CENTER_DEADBAND_PX and abs(error_y) <= CENTER_DEADBAND_PX:
            if target.get("distance_cm") is None:
                try:
                    raw = [
                        item for item in detector.detect(frame)
                        if str(item.get("letter", "")).upper() == reference_label
                        and item.get("distance_cm") is not None
                        and _task3_candidate_in_block(item, locked_block)
                    ]
                    if raw:
                        target["distance_cm"] = min(
                            raw,
                            key=lambda item: float(
                                np.linalg.norm(
                                    np.asarray(item.get("center", (cx, cy)), dtype=np.float32)
                                    - np.asarray((cx, cy), dtype=np.float32)
                                )
                            ),
                        ).get("distance_cm")
                except (AttributeError, cv2.error, TypeError, ValueError):
                    pass
            if target.get("distance_cm") is None and last_valid_distance is not None:
                target["distance_cm"] = last_valid_distance
            if target.get("distance_cm") is not None:
                try:
                    last_valid_distance = float(target["distance_cm"])
                except (TypeError, ValueError):
                    pass
            target["frame_shape"] = frame.shape
            target["center_id1"] = id1
            target["center_id2"] = id2
            target["center_id6"] = id6
            print(
                f"TASK3 TARGET CENTERED dx={error_x:.0f} dy={error_y:.0f} "
                f"ID1={id1} ID2={id2} ID6={id6} corrections={corrections}",
                flush=True,
            )
            return target

        next_id2 = id2
        next_id6 = id6
        if abs(error_x) > CENTER_DEADBAND_PX:
            next_id6 += -CENTER_STEP_TICKS if error_x > 0.0 else CENTER_STEP_TICKS
        if abs(error_y) > CENTER_DEADBAND_PX:
            next_id2 += -CENTER_STEP_TICKS if error_y > 0.0 else CENTER_STEP_TICKS
        next_id2 = max(CENTER_ID2_RANGE[0], min(CENTER_ID2_RANGE[1], next_id2))
        next_id6 = max(CENTER_ID6_RANGE[0], min(CENTER_ID6_RANGE[1], next_id6))
        if next_id2 == id2 and next_id6 == id6:
            target["frame_shape"] = frame.shape
            target["center_id1"] = id1
            target["center_id2"] = id2
            target["center_id6"] = id6
            print(
                f"TASK3 TARGET CENTER_LIMIT dx={error_x:.0f} dy={error_y:.0f} "
                f"ID1={id1} ID2={id2} ID6={id6} corrections={corrections}",
                flush=True,
            )
            return target

        id2, id6 = next_id2, next_id6
        boards.arm({2: id2, 6: id6}, CENTER_TIME_MS)
        corrections += 1
        print(
            f"TASK3 TARGET CENTER_STEP dx={error_x:.0f} dy={error_y:.0f} "
            f"ID2={id2} ID6={id6} step={CENTER_STEP_TICKS} "
            f"motion={CENTER_TIME_MS}ms",
            flush=True,
        )
        time.sleep(CENTER_TIME_MS / 1000.0)


def run(args) -> int:
    camera = boards = h7 = None
    started = False
    completed = False
    sequence = (int(time.time() * 1000.0)) & 0xFFFFFFFF
    grabbed: dict[str, int] = {}
    history: deque = deque(maxlen=TARGET_VOTE_WINDOW)
    try:
        detector = import_main_detector()
        grasp_model = _import_grasp_model()
        orbit_rescue_detector = _configure_orbit_rescue_detector(
            copy.copy(detector)
        )
        camera = open_camera(args.main_camera)
        print(
            f"TASK3 MAIN CAMERA_OPEN path={args.main_camera} "
            "ROLE=MAIN ONLY",
            flush=True,
        )
        if not args.execute_h7:
            print("TASK3 CAMERA_ONLY READY detector=ABCD", flush=True)
            deadline = time.monotonic() + args.camera_timeout_s
            while time.monotonic() < deadline:
                ok, frame = camera.read()
                if not ok or frame is None:
                    continue
                candidates = _letter_candidates(frame, detector, set(), MAIN_LETTER_MIN_CONFIDENCE)
                cv2.imshow(
                    "Task3 main",
                    _draw_view(frame, candidates, set(), "CAMERA_ONLY"),
                )
                if cv2.waitKey(1) & 0xFF in (ord("q"), 27):
                    break
            return 0

        boards = ServoBoards(args.arm_uart)
        _task3_pose_high(boards)
        print(
            "TASK3 ARM_HIGH ID1=650 ID2=570 ID6=415 "
            "ID14=300 ID15=600 ID17=320",
            flush=True,
        )
        h7 = Task3H7Link(args.h7_device)
        h7.open()
        field = args.field.upper()
        h7.send(
            f"RK,TEST,TASK3,START,SEQ,{sequence},FIELD,{field},"
            f"RADIUS_MM,{TASK3_ORBIT_RADIUS_MM},"
            f"ANGLE_DEG,{TASK3_ORBIT_ANGLE_DEG}"
        )
        h7.wait_status(sequence, "ACK", args.ack_timeout_s)
        h7.wait_status(sequence, "RUNNING", args.running_timeout_s)
        started = True
        print(
            f"TASK3 ORBIT_STARTED radius={TASK3_ORBIT_RADIUS_MM}mm "
            f"angle={TASK3_ORBIT_ANGLE_DEG}deg field={field}",
            flush=True,
        )

        orbit_frame_index = 0
        pending_block = None
        pending_block_frames = 0
        handled_blocks: list[dict] = []
        while True:
            for line in h7.poll():
                if h7._matches(line, "DONE", sequence):
                    completed = True
                    break
                if h7._matches(line, "ERR", sequence):
                    raise RuntimeError(f"H7_ORBIT_ERR {line}")
            if completed:
                break

            ok, frame = camera.read()
            if not ok or frame is None:
                time.sleep(0.01)
                continue
            blocks = _task3_parallel_blocks(frame, detector)
            orbit_frame_index += 1
            active_handled_blocks = []
            for handled in handled_blocks:
                still_visible = any(
                    _task3_block_tracks_match(
                        handled.get("block"), candidate
                    )
                    for candidate in blocks
                )
                if still_visible:
                    handled["missing_frames"] = 0
                    active_handled_blocks.append(handled)
                    continue
                handled["missing_frames"] = int(
                    handled.get("missing_frames", 0)
                ) + 1
                if handled["missing_frames"] < TASK3_HANDLED_BLOCK_REARM_FRAMES:
                    active_handled_blocks.append(handled)
                else:
                    print(
                        f"TASK3 BLOCK_REARMED label={handled.get('label', '?')} "
                        "reason=NOT_VISIBLE",
                        flush=True,
                    )
            handled_blocks = active_handled_blocks
            blocks = [
                candidate
                for candidate in blocks
                if not _task3_has_grabbed_letter(
                    frame, detector, candidate, grabbed
                )
                and not any(
                    _task3_block_tracks_match(
                        handled.get("block"), candidate
                    )
                    for handled in handled_blocks
                )
            ]
            block = max(
                blocks,
                key=lambda item: float(item.get("projected_area", 0.0)),
                default=None,
            )
            cv2.imshow(
                "Task3 main",
                _draw_view(frame, [], grabbed, "ORBIT / WHITE BLOCK"),
            )
            if cv2.waitKey(1) & 0xFF in (ord("q"), 27):
                raise KeyboardInterrupt
            if block is None:
                pending_block = None
                pending_block_frames = 0
                continue

            if _task3_blocks_match(pending_block, block):
                pending_block_frames += 1
            else:
                pending_block = block
                pending_block_frames = 1
            if pending_block_frames < TASK3_WHITE_BLOCK_CONFIRM_FRAMES:
                continue
            pending_block = None
            pending_block_frames = 0

            label = "?"
            history.clear()
            print(
                f"TASK3 WHITE_BLOCK_CONFIRMED angle={block.get('angle')} "
                f"center={block.get('center')} action=PAUSE_THEN_CLASSIFY",
                flush=True,
            )
            try:
                h7.pause_until_confirmed(sequence, field, args.pause_timeout_s)
            except Exception as exc:
                print(
                    f"TASK3 PAUSE_FAILED error={exc}; action=SAFE_HIGH_THEN_STOP",
                    flush=True,
                )
                _task3_stop_safely(h7, sequence, field, boards)
                raise
            print("TASK3 ORBIT_PAUSED reason=PARALLEL_WHITE_BLOCK", flush=True)

            target = _task3_classify_paused_block(
                camera,
                detector,
                orbit_rescue_detector,
                block,
                grabbed,
                TASK3_CLASSIFY_TIMEOUT_S,
            )
            if target is None:
                print(
                    "TASK3 BLOCK_CLASSIFY_NONE action=RESUME_ORBIT",
                    flush=True,
                )
                _task3_resume_or_recover(
                    h7, sequence, field, args.resume_timeout_s, boards,
                    "NO_ASSOCIATED_LETTER",
                )
                continue

            label = str(target.get("letter", "")).upper()
            if (
                label not in TASK3_LETTERS
                or grabbed.get(label, 0) >= TASK3_LETTER_SUCCESS_QUOTA
            ):
                _task3_record_handled_block(
                    handled_blocks,
                    label,
                    block,
                )
                print(
                    f"TASK3 BLOCK_CLASSIFY_SKIP letter={label} action=RESUME_ORBIT",
                    flush=True,
                )
                _task3_resume_or_recover(
                    h7, sequence, field, args.resume_timeout_s, boards,
                    "NON_TARGET_LETTER",
                )
                continue
            print(
                f"TASK3 BLOCK_CLASSIFIED letter={label} "
                f"depth_cm={target.get('distance_cm', '-')}",
                flush=True,
            )

            if not _task3_capture_authorized(target, block, grabbed):
                print(
                    "TASK3 CAPTURE_AUTHORIZATION_FAILED action=RESUME_ORBIT",
                    flush=True,
                )
                _task3_record_handled_block(handled_blocks, label, block)
                _task3_resume_or_recover(
                    h7, sequence, field, args.resume_timeout_s, boards,
                    "CLASSIFIED_TARGET_NOT_BOUND_TO_PAUSED_BLOCK",
                )
                continue

            try:
                centered = _center_task3_target(
                    camera, detector, orbit_rescue_detector, target, boards
                )
            except RuntimeError as exc:
                if not str(exc).startswith("TARGET_TRACK_TIMEOUT"):
                    raise
                # The orbit is already safely paused. If the confirmed target
                # disappears during the bounded re-centering window, release
                # the pause and keep scanning instead of leaving the chassis
                # stopped until the outer error handler sends STOP.
                print(
                    "TASK3 TARGET_LOST_AFTER_PAUSE "
                    f"missing_timeout={TARGET_TRACK_MISSING_TIMEOUT_S:.1f}s "
                    "action=RESUME_ORBIT",
                    flush=True,
                )
                _task3_resume_or_recover(
                    h7, sequence, field, args.resume_timeout_s, boards,
                    "TARGET_TRACK_TIMEOUT",
                )
                history.clear()
                print("TASK3 ORBIT_RESUMED reason=TARGET_TRACK_TIMEOUT", flush=True)
                continue
            _letter_grasp(boards, centered, grasp_model)
            _task3_record_handled_block(
                handled_blocks,
                label,
                block,
            )
            grabbed[label] = grabbed.get(label, 0) + 1
            history.clear()
            time.sleep(TARGET_RECHECK_DELAY_S)

            _task3_resume_or_recover(
                h7, sequence, field, args.resume_timeout_s, boards,
                "LETTER_GRASP_COMPLETE",
            )
            print(
                f"TASK3 ORBIT_RESUMED grabbed="
                f"{','.join(f'{key}:{value}' for key, value in sorted(grabbed.items()))}",
                flush=True,
            )

        print(
            f"TASK3 TEST COMPLETE orbit={TASK3_ORBIT_ANGLE_DEG}deg "
            f"radius={TASK3_ORBIT_RADIUS_MM}mm "
            f"grabbed="
            f"{','.join(f'{key}:{value}' for key, value in sorted(grabbed.items())) or 'none'}",
            flush=True,
        )
        return 0
    except KeyboardInterrupt:
        print("TASK3 ABORTED", flush=True)
        return 2
    except Exception as exc:
        print(f"TASK3 ERROR {exc}", flush=True)
        return 1
    finally:
        if h7 is not None and started and not completed:
            try:
                h7.send(
                    f"RK,TEST,TASK3,STOP,SEQ,{sequence},FIELD,{args.field.upper()}"
                )
                h7.wait_status(sequence, "STOPPED", 2.0)
            except Exception as exc:
                print(f"TASK3 STOP_RESULT {exc}", flush=True)
        if camera is not None:
            camera.release()
        if boards is not None:
            boards.close()
        if h7 is not None:
            h7.close()
        cv2.destroyAllWindows()


def main() -> int:
    parser = argparse.ArgumentParser(
        description="RoboCup task-three standalone orbit and ABCD test"
    )
    parser.add_argument("--field", choices=("red", "blue"), required=True)
    parser.add_argument("--main-camera", default=MAIN_CAMERA)
    parser.add_argument("--h7-device", default=H7_DEVICE)
    parser.add_argument("--arm-uart", default=ARM_DEVICE)
    parser.add_argument(
        "--zp-uart", default=None, help=argparse.SUPPRESS
    )  # kept for old launchers; the active app never opens it
    parser.add_argument("--execute-h7", action="store_true")
    parser.add_argument("--camera-timeout-s", type=float, default=30.0)
    parser.add_argument("--ack-timeout-s", type=float, default=4.0)
    parser.add_argument("--running-timeout-s", type=float, default=8.0)
    parser.add_argument("--pause-timeout-s", type=float, default=5.0)
    parser.add_argument("--resume-timeout-s", type=float, default=3.0)
    return run(parser.parse_args())


if __name__ == "__main__":
    raise SystemExit(main())
